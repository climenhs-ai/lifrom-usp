...
if api_key:
    ...
    if st.button("✨ 타겟 및 소구점 발굴하기"):
	    ...
	    ### 3. 찐친에게 영업하는 현실적인 카톡 영업 멘트 (보너스 💡)
                - 상황: 핏과 원단에 깐깐한 절친에게 이 제품을 사라고 카카오톡으로 추천하는 상황.
                - 톤앤매너: 친밀하고 자연스러운 반말. (단, 과도한 호들갑, 오버액션, 알 수 없는 비유는 절대 금지)
                - 🚨 [작성 및 양식 주의사항 - 소설 창작 및 헛소리 절대 금지!!]
                  - "애기가 지나갔다", "자동 규격", "면세점" 등 입력 정보에 없는 쓸데없는 배경 상황이나 기괴한 표현을 절대 지어내지 마세요.
                  - 1, 2, 3 같은 숫자 목차는 쓰지 마세요.
                  - 무조건 쟁여야 하는 이유 **5가지**를 각각 **엔터(줄바꿈) 두 번으로 분리된 문단**으로 작성하세요.
                  - 6번에 입력된 [경쟁사 단점 정보]와 3, 4번에 입력된 [제품 소재/디테일 팩트] (예: 일체형 패드, 어깨끈 안쪽 숨김, 5cm 볼륨 등) **오직 이 팩트들만** 문장에 자연스럽게 녹여서 깔끔하게 영업하세요.
                '''
